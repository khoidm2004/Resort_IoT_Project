...
app.get("/api/v1/update-led-status", async (req, res) => {
  ...
  try {
    const pointLEDStatus = new Point("led_status").intField(
      "status",
      numericStatus,
    ); // No tag
    ...
    logger.info({
      statusCode: 200,
      message: `LED status updated to ${status}`,
      timestamp: new Date().toISOString(),
    });
  } catch (err) {
    console.error(err);
    ...
    logger.error({
      statusCode: 500,
      message: "Error writing data to InfluxDB",
      timestamp: new Date().toISOString(),
      error: err.message,
    });
  }
      ...
      await DB_READ_API.queryRows(query, {
        ...
        complete() {
          if (results.length === 0) {
            ...
            res.write(
              `data: ${JSON.stringify({ status: ledStatus === 1 ? "on" : "off" })}\n\n`,
            );
          }
        },
      });
...
app.get("/api/v1/embed-temperature", async (req, res) => {
  ...
  try {
    const pointTemperature = new Point("qparams").floatField(
      "temperature",
      numeric_temperature,
    );
    ...
    logger.info({
      statusCode: 200,
      message: "Temperature recorded and uid recorded",
      timestamp: new Date().toISOString(),
    });
  } catch (err) {
    console.error(err);
...
app.get("/api/v1/embed-humidity", async (req, res) => {
  ...
  try {
    const pointHumidity = new Point("qparams").floatField(
      "humidity",
      numeric_humidity,
    );
    ...
    try {
      ...
      await DB_READ_API.queryRows(query, {
        ...
        error(error) {
          ...
          logger.error({
            statusCode: 500,
            message: "Error fetching data from InfluxDB",
            timestamp: new Date().toISOString(),
            error: error.message,
          });
        },
      ...
      await DB_READ_API.queryRows(query, {
        ...
        complete() {
          ...
          if (formattedResults.length === 0) {
            ...
            logger.info({
              statusCode: 200,
              message: "Humidity data fetched successfully",
              timestamp: new Date().toISOString(),
            });
          }
        },
...
app.post("/api/v1/upload", upload.single("image"), async (req, res) => {
  ...
  if (!style_of_image.includes(req.file.mimetype)) {
    ...
    return res
      .status(400)
      .send("Only image files are allowed (jpeg, png, gif, webp).");
  }
  try {
    ...
    const file_folder_name = bucket.file(
      `admin_avatar/${uid}${file_extension}`,
    );
    ...
    createStream.on("finish", () => {
      ...
      logger.info({
        statusCode: 200,
        message: `File uploaded successfully: ${publicUrl}`,
        timestamp: new Date().toISOString(),
      });
    });
...
app.get("/api/v1/update-relay-status", async (req, res) => {
  ...
  try {
    const pointLEDStatus = new Point("relay_status").intField(
      "status",
      numericStatus,
    );
    ...
    logger.info({
      statusCode: 200,
      message: `Relay status updated to ${status}`,
      timestamp: new Date().toISOString(),
    });
  } catch (err) {
    console.error(err);
...
app.post("/api/v1/upload_events", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    if (file) {
      ...
      const file_folder_name = bucket.file(
        `imageEvent/${req.file.originalname}`,
      );
